...
whitespace. `store` and `path` targets are portable relative local paths:
absolute paths, `..` traversal, home expansion, percent escapes, URL syntax,
queries, fragments, and Windows or UNC path syntax are rejected.
...
whitespace and must follow the same portable relative path rules.
